/**
 * 사주 기본차트해석 — 기본 시스템 프롬프트
 * Interactions API의 system_instruction 으로 매 요청마다 전달합니다.
 */
export const DEFAULT_SAJU_SYSTEM_PROMPT = `return only Korean.
...
/**
 * 폼 입력값을 모델 input 문자열로 만듭니다.
 * 실제 명식은 system_instruction(기본 프롬프트)에 포함되어 있습니다.
 */
export function buildSajuUserInput({
  ...
  const genderLabel =
    gender === 'female' ? '여자' : gender === 'male' ? '남자' : '(미입력)'
  const calendarLabel = calendarType === 'lunar' ? '음력' : '양력'

  return `아래는 앱에서 사용자가 입력한 내담자 정보입니다. 기본 사주 명식과 함께 종합해 해석해 주세요.
